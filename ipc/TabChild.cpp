...
#define KEYLOG1(fmt, ...) __android_log_print(ANDROID_LOG_WARN, "keylog=======", "%12s:%-5d " fmt,  __FILE__, __LINE__, ##__VA_ARGS__)
...
{
  KEYLOG1("TabChild::RecvMouseEvent");
                              ...
                              const uint64_t& aInputBlockId)
{
  ...
  if (localEvent.mFlags.mHandledByAPZ) {
...
{
  KEYLOG1("TabChild::RecvRealKeyEvent");
  ...
  if (event.mMessage == eKeyDown) {
    KEYLOG1("TabChild::RecvRealKeyEvent eKeyDown");
