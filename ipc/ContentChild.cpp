...
#define GPSLOG3(fmt, ...) __android_log_print(ANDROID_LOG_WARN, "GPS", "%12s:%-5d " fmt,  __FILE__, __LINE__, ##__VA_ARGS__)
...
{
    GPSLOG3("ContentChild::RecvGeolocationUpdate");
...
{
    GPSLOG3("ContentChild::RecvGeolocationError");
    nsCOMPtr<nsIGeolocationUpdate> gs = do_GetService("@mozilla.org/geolocation/service;1");
    if (!gs) {
        return true;
    }
