...
//adhoc
#include "mozilla/dom/adhoc/MozAdhoc.h"
#include "utils/Log.h"
...
NS_IMPL_CYCLE_COLLECTION_UNLINK(mAdhoc)
...
// 属性的实现函数
adhoc::MozAdhoc*
Navigator::GetMozAdhoc(ErrorResult& aRv)
{
 ALOGI("adhoc,Navigator, GetMozAdhoc");
  /*if(!CheckPermission("adhoc")){
  ALOGI("adhoc,Navigator,  GetMozAdhoc CheckPermission return false");
  aRv.Throw(NS_ERROR_UNEXPECTED);
  return nullptr;    
  }*/
  if(!mAdhoc){
    mAdhoc =new adhoc::MozAdhoc(mWindow);
  }
  else{
    ALOGI("Navigator, mAdhoc exist just return");
  }
  return mAdhoc;
}
