...
#define KEYLOG(fmt, ...) __android_log_print(ANDROID_LOG_WARN, "keylog=======", "%12s:%-5d " fmt,  __FILE__, __LINE__, ##__VA_ARGS__)
...
{
  KEYLOG("HardwareKeyHandler::ForwardKeyToInputMethodApp");
  ...
  if (aEvent->mMessage == eKeyPress && mEventQueue.IsEmpty()) {
    KEYLOG("HardwareKeyHandler::ForwardKeyToInputMethodApp  DispatchKeyPress");
  ...
  KEYLOG("HardwareKeyHandler::ForwardKeyToInputMethodApp  DispatchKeyPress isSent=%d",isSent);
...
{
  KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp");
  ...
  if(keyInfo->mEvent.mMessage == eKeyDown){
    KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp eKeyDown");
  }else if(keyInfo->mEvent.mMessage == eKeyUp){
    KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp eKeyUp");
  }
  ...
  if (CanDispatchEvent(keyInfo->mTarget, keyInfo->mEvent)) {
    KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp CanDispatchEvent");
    ...
    if (keyInfo->mEvent.mFlags.mDefaultPrevented) {
      KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp DispatchAfterKeyEvent");
    ...
    // so we need to dispatch it to its current event target.
      ...
      KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp DispatchToTargetApp");
  ...
  if (keyInfo->mEvent.mMessage != eKeyDown) {
    KEYLOG("HardwareKeyHandler::OnHandledByInputMethodApp not keydown return");
    return NS_OK;
  }
...
{
  KEYLOG("HardwareKeyHandler::DispatchKeyPress");
  MOZ_ASSERT(aTarget, "No target provided");
  MOZ_ASSERT(aEvent, "No event provided");
...
HardwareKeyHandler::DispatchToTargetApp(nsINode* aTarget,
                                        ...
                                        nsEventStatus& aStatus)
{
  KEYLOG("HardwareKeyHandler::DispatchToTargetApp");
  MOZ_ASSERT(aTarget, "No target provided");
  MOZ_ASSERT(aEvent, "No event provided");
  ...
  if (NS_WARN_IF(!currentTarget)) {
    KEYLOG("HardwareKeyHandler::DispatchToTargetApp no currentTarget");
    return false;
  }
  ...
  // in the new app.
  nsCOMPtr<nsPIDOMWindow> originalRootWindow = GetRootWindow(aTarget);
  nsCOMPtr<nsPIDOMWindow> currentRootWindow = GetRootWindow(currentTarget);
  if (currentRootWindow != originalRootWindow) {
    KEYLOG("HardwareKeyHandler::DispatchToTargetApp root window is changed");
  ...
  if (!presShell) {
    KEYLOG("HardwareKeyHandler::DispatchToTargetApp no presShell");
    return false;
  }

  if (!presShell->CanDispatchEvent(&aEvent)) {
    KEYLOG("HardwareKeyHandler::DispatchToTargetApp presShell not CanDispatchEvent");
    return false;
  }

  // In-process case: the event target is in the current process
  if (!PresShell::IsTargetIframe(currentTarget)) {
    KEYLOG("HardwareKeyHandler::DispatchToTargetApp presShell in the b2g process");
  ...
  KEYLOG("HardwareKeyHandler::DispatchToTargetApp presShell in the child process");
...
HardwareKeyHandler::DispatchToCurrentProcess(nsIPresShell* presShell,
                                             ...
                                             nsEventStatus& aStatus)
{
  KEYLOG("HardwareKeyHandler::DispatchToCurrentProcess");
...
HardwareKeyHandler::DispatchToCrossProcess(nsINode* aTarget,
                                           WidgetKeyboardEvent& aEvent)
{
  KEYLOG("HardwareKeyHandler::DispatchToCrossProcess");
  ...
  KEYLOG("HardwareKeyHandler::DispatchToCrossProcess remote->SendRealKeyEvent(aEvent)=%d",remote->SendRealKeyEvent(aEvent));
...
HardwareKeyHandler::PostHandleKeyboardEvent(nsINode* aTarget,
                                            ...
                                            nsEventStatus& aStatus)
{
  KEYLOG("HardwareKeyHandler::PostHandleKeyboardEvent");
  MOZ_ASSERT(aTarget, "No target provided");
  MOZ_ASSERT(aEvent, "No event provided");
...
HardwareKeyHandler::GetRootWindow(nsINode* aNode)
{
  ...
  nsCOMPtr<nsPIDOMWindow> rootWindow = presShell->GetRootWindow();
...
{
  KEYLOG("HardwareKeyHandler::GetCurrentTarget");
  ...
  if (NS_WARN_IF(!ourWindow)) {
    KEYLOG("HardwareKeyHandler::GetCurrentTarget no ourWindow return");
    return nullptr;
  }

  //nsCOMPtr<nsPIDOMWindow> rootWindow = ourWindow->GetPrivateRoot();
